...
# define EVP_MAX_MD_SIZE                 NUM_ECC_BYTES
...
typedef struct EC_KEY{
    uECC_Curve curve;
    unsigned int pub_x[NUM_ECC_WORDS];
    unsigned int pub_y[NUM_ECC_WORDS];
    unsigned int priv[NUM_ECC_WORDS];
}EC_KEY;

typedef struct ECDSA_SIG{
    unsigned int r[NUM_ECC_WORDS];
    unsigned int s[NUM_ECC_WORDS];
}ECDSA_SIG;


EC_KEY* EC_KEY_new(){
    EC_KEY* newKey = malloc(sizeof (EC_KEY));
    memset(newKey,0,sizeof (EC_KEY));
...
EC_KEY * ECKey_From(const cn_cbor * pKey, size_t * cbGroup, cose_errback * perr)
{
...
/*
    ...
    CHECK_CONDITION(((size_t)p->length) == *cbGroup, COSE_ERR_INVALID_PARAMETER);
    //memcpy(rgbKey+1, p->v.str, p->length);
    memcpy(pNewKey->pub_x, p->v.str, p->length);
    ...
    if (p->type == CN_CBOR_BYTES) {
        ...
        memcpy(pNewKey->pub_y, p->v.str, p->length);
    }
    ...
    */
    ...
    if (p != NULL) {
        memcpy(pNewKey->priv, p->v.bytes, p->length);
        //BIGNUM * pbn;
...
typedef void (mbedtls_sha_func)( const unsigned char *input, size_t ilen,
             unsigned char* output,unsigned* outputSize);
void mbedtls_sha256_func( const unsigned char *input, size_t ilen,unsigned char* output,unsigned* outputSize){
    mbedtls_sha256(input,ilen,output,0);
    *outputSize = 32;
}
void mbedtls_sha512_func( const unsigned char *input, size_t ilen,unsigned char* output,unsigned* outputSize){
    mbedtls_sha512(input,ilen,output,0);
    *outputSize = 48;
}
void mbedtls_sha384_func( const unsigned char *input, size_t ilen, unsigned char* output,unsigned* outputSize){
...
bool ECDSA_Verify(COSE * pSigner, int index, const cn_cbor * pKey, int cbitDigest, const byte * rgbToSign, size_t cbToSign, cose_errback * perr){
    ...
    memcpy(sig.r,pSig->v.bytes,(int) cbSignature/2);
    memcpy(sig.s,pSig->v.bytes+cbSignature/2,(int) cbSignature/2);

    CHECK_CONDITION(uECC_verify((uint8_t*)eckey->pub_x,rgbDigest, cbDigest, (uint8_t*)sig.r,eckey->curve) == 1, COSE_ERR_CRYPTO_FAIL);
